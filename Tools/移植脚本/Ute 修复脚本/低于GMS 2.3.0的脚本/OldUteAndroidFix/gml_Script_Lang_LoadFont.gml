...
if (!file_exists("locale/" + LANG + "/" + "font.txt"))
    return 0;

var LIST = Lang_LoadFileToString("locale/" + LANG + "/" + "font.txt");
...
while (!file_text_eof(FILE))
{
    ...
    if (file_exists(PATH))
    {
        ...
        var is_sprite = ini_read_real("font", "is_sprite", 0);
        var size = ini_read_real("font", "size", 12);
        var bold = ini_read_real("font", "bold", 0);
        var italic = ini_read_real("font", "italic", 0);
        var first = ini_read_real("font", "first", 32);
        var last = ini_read_real("font", "last", 128);
        var string_map = ini_read_string("font", "string_map", "");
        var is_proportional = ini_read_real("font", "is_proportional", 0);
        ...
        if (key != "")
        {
            ...
            else if (Lang_IsSpriteExists(src))
            {
                if (string_map == "")
